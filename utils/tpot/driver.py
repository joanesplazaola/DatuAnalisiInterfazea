...
def _get_arg_parser():
	...
	parser.add_argument(
		...
		type=bool,
		help=(
			'Whether the expected output is multi-dimensional or not.'
		))
	...
	parser.add_argument(
		...
		default=['class'],
		type=str,
		help='Name of the target column(s) in the input file.'
	)
...
def _read_data_file(args):
	...
	if any(target not in input_data.columns.values for target in args.TARGET_NAMES):
		raise ValueError(
...
def tpot_driver(args):
	...
	features = input_data.drop(args.TARGET_NAMES, axis=1)

	training_features, testing_features, training_target, testing_target = \
		train_test_split(features, input_data[args.TARGET_NAMES], random_state=args.RANDOM_STATE)
